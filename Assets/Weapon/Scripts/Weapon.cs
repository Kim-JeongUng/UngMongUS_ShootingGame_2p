...
public class Weapon : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (this.transform.parent != null) // @ my code
		{
			CheckForUserInput();
		}
	...
	{ 
		// @ my code
		bool checkInput1p = ((Input.GetButton("Fire1")) && (this.transform.parent.tag == "EquipPoint1"));
		bool checkInput2p = ((Input.GetButton("Fire2")) && (this.transform.parent.tag == "EquipPoint2"));
		...
		if (type == WeaponType.Raycast)
		{
			...
			{ // @ my code
				if (checkInput1p || checkInput2p)  //사용가능한 총만 발사
				{
				...
				if ((warmup && checkInput1p) || (warmup && checkInput2p))
				{
		...
		if (type == WeaponType.Beam)
		{
			if ((checkInput1p && beamHeat <= maxBeamHeat && !coolingDown) || (checkInput2p && beamHeat <= maxBeamHeat && !coolingDown)) //사용가능한 총만 발사
			{
				Beam();
			}
			else
		...
		for (int i = 0; i < shotPerRound; i++)
		{
			...
			if (Physics.Raycast(ray, out hit, range))
			{
				...
				if (makeBulletHoles && !exception)
				{
					...
					if (hit.collider.gameObject.tag == "Player1" || hit.collider.gameObject.tag == "Player2") //@ my code raycast데미지 
					{
						var call1 = hit.transform.root.GetComponent<player_move>();
						call1.nowHp = call1.nowHp - (int)(damage / 10);
					}
				}
	...
	void Beam()
	{
		...
		do
		{
			...
			if (Physics.Raycast(ray, out hit, range))
			{
				...
				{
					var call1 = hit.transform.root.GetComponent<player_move>();
					call1.nowHp = call1.nowHp - (int)(beamPower);
				}
